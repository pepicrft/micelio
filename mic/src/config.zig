//! Local configuration management for mic.
//!
//! Configuration is stored in `~/.mic/config.json` (or `$MIC_HOME/config.json`
...
/// Get the mic configuration directory path.
/// Uses $MIC_HOME if set, otherwise ~/.mic
pub fn configDir(allocator: std.mem.Allocator) ![]u8 {
    if (std.process.getEnvVarOwned(allocator, "MIC_HOME")) |value| {
    ...
    return std.fs.path.join(allocator, &.{ home, ".mic" });
}
