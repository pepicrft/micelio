...
const fs_server = @import("mic/fs/server.zig");
const state_mod = @import("mic/fs/state.zig");
...
fn usage() !void {
    std.debug.print(
        "Usage: mic-fs --server <grpc_url> --account <org> --project <project> --state-dir <path> [--nfs-port <port>] [--mount-port <port>]\n",
        .{},
    );
