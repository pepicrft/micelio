...
The Session Manager API defines how Micelio requests, tracks, and connects to
sandboxed sessions for agents, validation, review, and CI workloads.
...
- Transport: HTTP + JSON (gRPC is allowed as long as payloads match).
...
- Session managers normalize incoming payloads into the canonical shape.
- Invalid values return structured errors (invalid state, access type, timestamp).
- Optional callbacks include `list/1`, `extend/2`, and `heartbeat/1`.
