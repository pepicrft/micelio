//! mic library entry points.
//!
//! This is the main entry point for the mic library, providing access to:
//! - Core algorithms (hash, hlc, bloom, tree) via their respective namespaces
//!
//! Note: mic uses a forge-first architecture where repositories exist on the
//! server (forge), not locally. There is no local repository initialization.
//! Use `mic clone <repo>` to set up local state for working with a forge repo.
