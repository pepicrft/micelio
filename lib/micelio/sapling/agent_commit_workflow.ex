defmodule Micelio.Sapling.AgentCommitWorkflow do
  @moduledoc """
  Proof-of-concept for mapping agent session commits into Git/Sapling workflows.
  """
  ...
  def run(opts \\ []) do
    ...
    session_ids = Keyword.get(opts, :session_ids, build_session_ids(sessions))
    ...
    results =
      Enum.map(availability.available, fn tool ->
        run_tool(tool, session_ids, tmp_root, runner, fs, cleanup)
      end)
  ...
  def format_markdown(report) do
    ...
    session_ids = Map.get(report, :session_ids, [])
    started_at = Map.get(report, :started_at, DateTime.utc_now())

    header =
      [
        "# Sapling agent commit workflow report",
        "",
        "This report is generated by `mix micelio.sapling.agent_commit_workflow`.",
        "",
      ...
      |> Kernel.++([
        "",
        "Session IDs: `#{Enum.join(session_ids, ", ")}`",
        "",
        "## Workflow",
        "",
        "- Initialize a repo per tool",
        "- Create a base commit",
        "- Create agent session commits with `Mic-Session` trailers",
        "- Capture stack/log views and full commit messages"
      ])
  ...
  defp run_tool(tool, session_ids, tmp_root, runner, fs, cleanup) do
    repo_path =
      Path.join(
        tmp_root,
        "sapling_agent_commit_workflow_#{tool}_#{System.unique_integer([:positive])}"
      )
  ...
  defp create_session_commits({steps, :ok}, tool, session_ids, repo_path, env, runner, fs) do
    Enum.reduce(session_ids, {steps, :ok}, fn session_id, acc ->
      ...
      if current_status == :error do
        ...
        {current_steps, :ok}
        |> write_session_file(repo_path, session_id, fs)
        |> run_step(:stage_session, tool_command(tool), add_args(tool), repo_path, env, runner)
        |> run_step(
          :commit_session,
          tool_command(tool),
          commit_args(tool, session_message(session_id)),
          repo_path,
          env,
          runner
        )
  ...
  defp write_session_file({steps, :ok}, repo_path, session_id, fs) do
    ...
    fs.write!(
      Path.join(notes_dir, "#{session_id}.md"),
      "Session #{session_id}\n\nProof-of-concept agent change.\n"
    )
  ...
  defp session_message(session_id) do
    [
      "agent: apply session #{session_id}",
      "",
      "Mic-Session: #{session_id}",
      "Mic-Actor: codex",
      "Mic-Intent: proof-of-concept"
    ]
    |> Enum.join("\n")
  end
  ...
  defp message_view_args(:git, session_ids) do
    ["log", "-n", Integer.to_string(length(session_ids) + 1), "--pretty=fuller"]
  end

  defp message_view_args(:sapling, session_ids) do
    ["log", "-l", Integer.to_string(length(session_ids) + 1)]
  end

  defp build_session_ids(count) do
    Enum.map(1..count, &"session-#{&1}")
  end
end
