defmodule Micelio.Authorization.Checks do
  ...
  def project_member(%User{} = user, %Account{} = account),
    do: Accounts.user_has_account_access?(user, account)

  def project_member(%User{} = user, %{account: %Account{} = account}),
    do: Accounts.user_has_account_access?(user, account)

  def project_member(%User{} = user, %{organization_id: organization_id}),
    do: Accounts.user_in_organization?(user, organization_id)

  def project_member(_, _), do: false

  def project_admin(%User{} = user, %Account{} = account),
    do:
      Accounts.user_owns_account?(user, account) or
        Accounts.user_role_in_organization?(user, account.organization_id, :admin)

  def project_admin(%User{} = user, %{account: %Account{} = account}),
    do: project_admin(user, account)

  def project_admin(%User{} = user, %{organization_id: organization_id}),
    do: Accounts.user_role_in_organization?(user, organization_id, :admin)

  def project_admin(_, _), do: false
end
