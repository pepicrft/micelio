...
pub fn ensureMetadataDir(workspace_root: []const u8) !void {
    const base = try std.fs.path.join(std.heap.page_allocator, &[_][]const u8{ workspace_root, ".hif" });
...
fn manifestPath(allocator: std.mem.Allocator, workspace_root: []const u8) ![]u8 {
    return std.fs.path.join(allocator, &[_][]const u8{ workspace_root, ".hif", manifest_filename });
}
