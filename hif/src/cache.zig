...
fn cacheDir(allocator: std.mem.Allocator) ![]u8 {
    ...
    return std.fs.path.join(allocator, &.{ home, ".cache", "hif", "blobs" });
}
...
test "cacheDir returns valid path" {
    ...
    try std.testing.expect(std.mem.endsWith(u8, dir, "hif/blobs"));
}
