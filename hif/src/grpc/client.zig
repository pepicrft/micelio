...
pub fn unaryCall(
    ...
    const result = try unaryCallResult(allocator, target, host, method, request, auth_token);
...
pub fn unaryCallResult(
    ...
    const target_z = try toNullTerminated(allocator, target);
    defer allocator.free(target_z);
    const host_z = try toNullTerminated(allocator, host);
